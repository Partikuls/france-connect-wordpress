 ...
 *
 * @wordpress-plugin
 * Plugin Name:       France Connect WordPress
 * Plugin URI:        https://github.com/Partikuls/france-connect-wordpress
 * Description:       Connect to an France Connect identity provider using Authorization Code Flow.
 * Version:           1.0.0
 * Requires at least: 5.0
 * Requires PHP:      7.4
 * Author:            partikuls
 * Author URI:        http://www.partikuls.com
 * Text Domain:       france-connect-openid-connect-generic
 * Domain Path:       /languages
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 * GitHub Plugin URI: https://github.com/Partikuls/france-connect-wordpress
 */
...
Notes
  ...
  - openid-connect-generic-state-expired                   - the given state exists, but expired before this login attempt.
	- openid-connect-generic-nonce-not-found                 - the given nonce does not exist in the database, regardless of its expiration.
  - openid-connect-generic-nonce-expired                   - the given nonce exists, but expired before this login attempt.
...
class OpenID_Connect_Generic {
	...
	public function enforce_privacy_feeds( $content ) {
		if ( $this->settings->enforce_privacy && ! is_user_logged_in() ) {
			$content = __( 'Private site', 'france-connect-openid-connect-generic' );
		}
		return $content;
	}
