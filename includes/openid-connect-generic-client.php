...
class OpenID_Connect_Generic_Client {
	...
	public function validate_authentication_request( $request ) {
		$this->logger->log( $request, 'validate_authentication_request' );
		...
		if ( ! isset( $request['state'] ) ) {
			...
			return new WP_Error( 'missing-state', __( 'Missing state.', 'france-connect-openid-connect-generic' ), $request );
		}

		if ( ! $this->check_state( $request['state'] ) ) {
			return new WP_Error( 'invalid-state', __( 'Invalid state.', 'france-connect-openid-connect-generic' ), $request );
		}
	...
	public function get_authentication_code( $request ) {
		if ( ! isset( $request['code'] ) ) {
			return new WP_Error( 'missing-authentication-code', __( 'Missing authentication code.', 'france-connect-openid-connect-generic' ), $request );
		}
	...
	public function request_authentication_token( $code ) {
		...
		$this->logger->log( $response, 'request_authentication_token', $end_time - $start_time );

		if ( is_wp_error( $response ) ) {
			$response->add( 'request_authentication_token', __( 'Request for authentication token failed.', 'france-connect-openid-connect-generic' ) );
		}
	...
	public function request_new_tokens( $refresh_token ) {
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'refresh_token', __( 'Refresh token failed.', 'france-connect-openid-connect-generic' ) );
		}
	...
	public function get_token_response( $token_result ) {
		if ( ! isset( $token_result['body'] ) ) {
			return new WP_Error( 'missing-token-body', __( 'Missing token body.', 'france-connect-openid-connect-generic' ), $token_result );
		}
		...
		if ( is_null( $token_response ) ) {
			return new WP_Error( 'invalid-token', __( 'Invalid token.', 'france-connect-openid-connect-generic' ), $token_result );
		}
	...
	public function request_userinfo( $access_token ) {
		...
		$this->logger->log( $request, 'request_userinfo', $end_time - $start_time );

		if ( is_wp_error( $response ) ) {
			$response->add( 'request_userinfo', __( 'Request for userinfo failed.', 'france-connect-openid-connect-generic' ) );
		}
	...
	/**
	 * Generate a new state and nonce save them as a transient, and return the state hash.
	 *
	...
	public function new_state( $redirect_to ) {
		...
		//$state = bin2hex(random_bytes(11));
		$nonce = md5( mt_rand() . microtime( true ) );
		$state_value = array(
			$state => array(
				'redirect_to' => $redirect_to,
				'nonce'       => $nonce  // Store the nonce in the state transient
			),
		);
	...
	/**
	 * Generate a new nonce and nonce save them as a transient, and return the nonce hash.
	 *
	 * @param string $redirect_to The redirect URL to be used after IDP authentication.
	 *
	 * @return string
	...
	public function get_nonce( $state ) {
		$state_value = get_transient( 'openid-connect-generic-state--' . $state );
		$nonce = $state_value[$state]['nonce'];
		return $nonce;
	}
	 ...
	 * @return bool
	 */
	public function validate_nonce( $state, $id_token_claim ) {
		
		if( !isset( $id_token_claim['nonce'] ) && empty( $id_token_claim['nonce'] )) 	{
			return new WP_Error( 'invalid-authentication-nonce', __( 'Invalid authentication nonce.', 'france-connect-openid-connect-generic' ), $id_token_claim );
		}
		
		$nonce = $id_token_claim['nonce'];
		$saved_nonce = $this->get_nonce( $state );
		if( $saved_nonce != $nonce ) {
			return new WP_Error( 'invalid-authentication-nonce', __( 'Invalid authentication nonce.', 'france-connect-openid-connect-generic' ), $nonce );
		}
		return true;
	}
	...
	public function get_authentication_state( $request ) {
		if ( ! isset( $request['state'] ) ) {
			return new WP_Error( 'missing-authentication-state', __( 'Missing authentication state.', 'france-connect-openid-connect-generic' ), $request );
		}
	...
	public function get_id_token_claim( $token_response ) {
		...
		if ( ! isset( $token_response['id_token'] ) ) {
			return new WP_Error( 'no-identity-token', __( 'No identity token.', 'france-connect-openid-connect-generic' ), $token_response );
		}
		...
		if ( ! isset( $tmp[1] ) ) {
			return new WP_Error( 'missing-identity-token', __( 'Missing identity token.', 'france-connect-openid-connect-generic' ), $token_response );
		}
	...
	public function validate_id_token_claim( $id_token_claim ) {
		if ( ! is_array( $id_token_claim ) ) {
			return new WP_Error( 'bad-id-token-claim', __( 'Bad ID token claim.', 'france-connect-openid-connect-generic' ), $id_token_claim );
		}

		// Validate the identification data and it's value.
		if ( ! isset( $id_token_claim['sub'] ) || empty( $id_token_claim['sub'] ) ) {
			return new WP_Error( 'no-subject-identity', __( 'No subject identity.', 'france-connect-openid-connect-generic' ), $id_token_claim );
		}
		...
		if ( ! empty( $this->acr_values ) && isset( $id_token_claim['acr'] ) ) {
			if ( $this->acr_values != $id_token_claim['acr'] ) {
				return new WP_Error( 'no-match-acr', __( 'No matching acr values.', 'france-connect-openid-connect-generic' ), $id_token_claim );
			}
		}
	...
	public function get_user_claim( $token_response ) {
		...
		$this->logger->log( $user_claim_result, 'get_user_claim' );
		...
		if ( is_wp_error( $user_claim_result ) || ! isset( $user_claim_result['body'] ) ) {
			return new WP_Error( 'bad-claim', __( 'Bad user claim.', 'france-connect-openid-connect-generic' ), $user_claim_result );
		}
	...
	public function validate_user_claim( $user_claim, $id_token_claim ) {
		...
		if ( ! is_array( $user_claim ) ) {
			return new WP_Error( 'invalid-user-claim', __( 'Invalid user claim.', 'france-connect-openid-connect-generic' ), $user_claim );
		}

		// Allow for errors from the IDP.
		if ( isset( $user_claim['error'] ) ) {
			$message = __( 'Error from the IDP.', 'france-connect-openid-connect-generic' );
		...
		if ( $id_token_claim['sub'] !== $user_claim['sub'] ) {
			return new WP_Error( 'incorrect-user-claim', __( 'Incorrect user claim.', 'france-connect-openid-connect-generic' ), func_get_args() );
		}
		...
		if ( ! $login_user ) {
			return new WP_Error( 'unauthorized', __( 'Unauthorized access.', 'france-connect-openid-connect-generic' ), $login_user );
		}
