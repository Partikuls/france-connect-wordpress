...
class OpenID_Connect_Generic_Client_Wrapper {
	...
	public function get_authentication_url( $atts = array() ) {
		...
		$url_format = '%1$s%2$sresponse_type=code&scope=%3$s&client_id=%4$s&state=%5$s&nonce=%6$s&redirect_uri=%7$s';
		if ( ! empty( $atts['acr_values'] ) ) {
			$url_format .= '&acr_values=%8$s';
		}
		...
		if ( ! $refresh_token || ( $refresh_expires && $current_time > $refresh_expires ) ) {
			if ( isset( $_SERVER['REQUEST_URI'] ) ) {
				...
				if ( $this->settings->redirect_on_logout ) {
					$this->error_redirect( new WP_Error( 'access-token-expired', __( 'Session expired. Please login again.', 'france-connect-openid-connect-generic' ) ) );
				}
	...
	public function get_end_session_logout_redirect_url( $redirect_url, $requested_redirect_to, $user ) {
		$this->logger->log( $redirect_url, 'get_end_session_logout_redirect_url' );
		$this->logger->log( $requested_redirect_to, 'get_end_session_logout_redirect_url requested_redirect_to' );
		$this->logger->log( $user, 'get_end_session_logout_redirect_url user' );
		...
		if ( isset( $claim['iss'] ) && 'https://accounts.google.com' == $claim['iss'] ) {
			...
			$this->logger->log( $url . sprintf( 'id_token_hint=%s&post_logout_redirect_uri=%s', $token_response['id_token'], urlencode( $redirect_url ) ), 'get_end_session_logout_redirect_url redirect' );
	...
	public function authentication_request_callback() {
		...
		$this->logger->log( $_GET, 'authentication_request_callback' );
		...
		$this->logger->log( $id_token_claim, 'id_token_claim' );
		...
		$valid_nonce = $client->validate_nonce( $state, $id_token_claim );
		if( is_wp_error( $valid_nonce ) ) {
			//$this->error_redirect( $valid_nonce );
			$this->logger->log( $valid_nonce, 'validate nonce' );
			$redirect_url = $this->get_end_session_logout_redirect_url(home_url('/connexion'), home_url('/connexion'), wp_get_current_user());
			// Redirect user back to login page.
			$this->logger->log( $this->settings->endpoint_end_session . '?id_token_hint=' . $token_response['id_token'] . '&state=' . $state . '&post_logout_redirect_uri=' . urlencode($redirect_url), 'FC deconnect url' );
			wp_redirect($redirect_url);
			wp_redirect($this->settings->endpoint_end_session . '?id_token_hint=' . $token_response['id_token'] . '&state=' . $state . '&post_logout_redirect_uri=' . urlencode($redirect_url));
			//wp_redirect($redirect_url);
			exit;
		}
		...
		if ( ! $user ) {
			...
			if ( $this->settings->link_existing_users || $this->settings->create_if_does_not_exist ) {
				...
				$this->error_redirect( new WP_Error( 'identity-not-map-existing-user', __( 'User identity is not linked to an existing WordPress user.', 'france-connect-openid-connect-generic' ), $user_claim ) );
			}
		}
	...
	public function validate_user( $user ) {
		...
		if ( ! is_a( $user, 'WP_User' ) || ! $user->exists() ) {
			return new WP_Error( 'invalid-user', __( 'Invalid user.', 'france-connect-openid-connect-generic' ), $user );
		}
	...
	private function get_username_from_claim( $user_claim ) {
		...
		if ( empty( $desired_username ) ) {
			...
			return new WP_Error( 'no-username', __( 'No appropriate username found.', 'france-connect-openid-connect-generic' ), $user_claim );
		}
		...
		if ( empty( $sanitized_username ) ) {
			...
			return new WP_Error( 'username-sanitization-failed', sprintf( __( 'Username %1$s could not be sanitized.', 'france-connect-openid-connect-generic' ), $desired_username ), $desired_username );
		}
	...
	private function get_nickname_from_claim( $user_claim ) {
		...
		if ( empty( $desired_nickname ) ) {
			...
			return new WP_Error( 'no-nickname', sprintf( __( 'No nickname found in user claim using key: %1$s.', 'france-connect-openid-connect-generic' ), $this->settings->nickname_key ), $this->settings->nickname_key );
		}
				...
				if ( ! $this->get_claim( $key, $user_claim, $info ) ) {
					if ( $error_on_missing_key ) {
						return new WP_Error(
							...
							__( 'User claim incomplete.', 'france-connect-openid-connect-generic' ),
							array(
	...
	public function create_new_user( $subject_identity, $user_claim ) {
		...
		if ( $values_missing && isset( $user_claim['access_token'] ) && ! empty( $this->settings->endpoint_userinfo ) ) {
			...
			if ( is_wp_error( $user_claim_result ) ) {
				return new WP_Error( 'bad-user-claim-result', __( 'Bad user claim result.', 'france-connect-openid-connect-generic' ), $user_claim_result );
			}
		...
		if ( ! $create_user ) {
			return new WP_Error( 'cannot-authorize', __( 'Can not authorize.', 'france-connect-openid-connect-generic' ), $create_user );
		}
		...
		$user_data = array(
			...
			'role' => $this->settings->role_mapping ? $this->settings->role_mapping : 'subscriber',
		);

		$user_data = apply_filters( 'openid-connect-generic-alter-user-data', $user_data, $user_claim );

		$this->logger->log( $user_data, 'user_data' );
		...
		if ( is_wp_error( $uid ) ) {
			return new WP_Error( 'failed-user-creation', __( 'Failed user creation.', 'france-connect-openid-connect-generic' ), $uid );
		}
