...
class OpenID_Connect_Generic_Login_Form {
	...
	public static function register( $settings, $client_wrapper ) {
		...
		wp_enqueue_style( 'france-connect-openid-connect-generic-public', plugin_dir_url( __DIR__ ) . 'css/styles.css', array(), OpenID_Connect_Generic::VERSION, 'all' );
	...
	public function make_error_output( $error_code, $error_message ) {
		...
		<div id="login_error"><?php // translators: %1$s is the error code from the IDP. ?>
			<strong><?php printf( esc_html__( 'ERROR (%1$s)', 'france-connect-openid-connect-generic' ), esc_html( $error_code ) ); ?>: </strong>
	...
	public function make_login_button( $atts = array() ) {
		...
		$atts = shortcode_atts(
			array(
				'button_text' => __( 'Login with OpenID Connect', 'france-connect-openid-connect-generic' ),
			),
			$atts,
...
<div class="openid-connect-login-button" style="margin: 1em 0; text-align: center;">
	<a class="button button-large" href="{$href}"></a>
</div>
HTML;
